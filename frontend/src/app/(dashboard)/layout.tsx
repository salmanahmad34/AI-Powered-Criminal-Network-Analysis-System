  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <aside
        ...
        } transition-all duration-300 ease-in-out bg-[var(--sidebar-bg)] border-r border-border flex flex-col z-20 shrink-0`}
      >
        {/* Brand */}
        ...
        <nav className="flex-1 px-4 py-6 space-y-6 overflow-y-auto">
          ...
          <div className="space-y-1">
            <span className="px-4 text-[10px] font-extrabold text-stone-400 tracking-wider uppercase block mb-2">Intelligence</span>
            {visibleMenuItems
              .filter(item => !['Admin Panel', 'Audit Logs'].includes(item.name))
              .map((item) => {
                const isActive = pathname === item.path || pathname?.startsWith(item.path + '/');
                // Clean name references matching specifications
          ...
          {visibleMenuItems.some(item => ['Admin Panel', 'Audit Logs'].includes(item.name)) && (
            <div className="space-y-1">
              <span className="px-4 text-[10px] font-extrabold text-stone-400 tracking-wider uppercase block mb-2">Administration</span>
              {visibleMenuItems
                .filter(item => ['Admin Panel', 'Audit Logs'].includes(item.name))
                .map((item) => {
                  const isActive = pathname === item.path || pathname?.startsWith(item.path + '/');
              ...
              {/* Settings placeholder matching list */}
              <div className="flex items-center gap-3 px-4 py-2.5 rounded-lg text-xs font-semibold text-stone-400 cursor-not-allowed opacity-50">
                <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.75">
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        {/* Header */}
        <header className="h-14 bg-white border-b border-[var(--border)] flex items-center justify-between px-6 z-10 shrink-0">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className="p-1.5 text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-muted)] rounded-md cursor-pointer transition-all"
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50 border border-emerald-200 rounded-full">
              ...
              <span className="text-[10px] font-semibold text-emerald-700 tracking-wide">DEMO DATABASE</span>
            </div>
            <span className="text-[10px] font-bold text-[var(--text-tertiary)] tracking-widest uppercase">DEV PORTAL</span>
          </div>
        </header>

        {/* Content pane */}
        <main className="flex-1 overflow-y-auto p-8 bg-background" style={{ paddingTop: '2rem' }}>
          {children}
        </main>
      </div>
    </div>
